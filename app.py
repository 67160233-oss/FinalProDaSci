...
with col1:
    # --- แก้ไขให้โชว์ภาษาไทย แต่ส่งค่าภาษาอังกฤษให้โมเดล ---
    channel_dict = {
        "ขับรถรับสินค้า (Drive-Thru)": "Drive-Thru",
        "สั่งผ่านแอปมือถือ (Mobile App)": "Mobile App",
        "สั่งหน้าเคาน์เตอร์ (In-Store)": "In-Store",
        "สั่งผ่านตู้หน้าจอสัมผัส (Kiosk)": "Kiosk"
    }
    selected_channel = st.selectbox("ช่องทางการสั่งซื้อ", list(channel_dict.keys()))
    order_channel = channel_dict[selected_channel] # เก็บค่าภาษาอังกฤษไว้ส่งให้โมเดล

    location_dict = {
        "ในเมือง / ย่านธุรกิจ (Urban)": "Urban",
        "ชานเมือง / แหล่งที่อยู่อาศัย (Suburban)": "Suburban",
        "นอกเมือง / ทางผ่าน (Rural)": "Rural"
    }
    selected_location = st.selectbox("ทำเลที่ตั้งสาขา", list(location_dict.keys()))
    store_location_type = location_dict[selected_location] # เก็บค่าภาษาอังกฤษไว้ส่งให้โมเดล
    # -----------------------------------------------------
...
if st.button("✨ ทำนายยอดใช้จ่าย", type="primary", use_container_width=True):
    ...
    input_data = pd.DataFrame([{
        'order_channel': order_channel, # ตัวแปรนี้จะกลายเป็นภาษาอังกฤษอัตโนมัติ
        'store_location_type': store_location_type, # ตัวแปรนี้จะกลายเป็นภาษาอังกฤษอัตโนมัติ
        'region': region,
